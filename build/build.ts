...
import { homePage, visionPage, placeholderPage } from './pages.ts';
...
async function renderDocs(): Promise<{
	...
	const relFiles = absFiles.map((f) => path.relative(DOCS_DIR, f).split(path.sep).join('/'));
	...
	for (const section of sections) {
		for (const snap of section.snapshots) {
			...
			const { title, html } = await renderMarkdown(source, section.dir, sectionIds);
			htmlBySnap.set(`${section.id}@${snap.versionStr}`, html);
			titleBySnap.set(`${section.id}@${snap.versionStr}`, title ?? section.name);
		}
	}
	const titleOf = (id: string, ver: string) => titleBySnap.get(`${id}@${ver}`) ?? id;
	const htmlOf = (id: string, ver: string) => htmlBySnap.get(`${id}@${ver}`) ?? '';
	...
	const renderTo = async (opts: {
		...
		const docTitle = titleOf(opts.currentId, versionToString(opts.viewedVersion));
		const docDesc = metaDescription(htmlOf(opts.currentId, versionToString(opts.viewedVersion)));
		await writePage(
			opts.urlPath,
			page({
				title: docTitle,
...
export async function build(): Promise<void> {
	...
	const homeDesc = 'Logos is a self-hosting systems programming language built on radical unification: programs, types, proofs, the optimizer, the standard library, and the compiler itself all live in one reflectable structure, the Logic Graph.';
	const examplesDesc = 'Worked Logos examples are on the way, showing one structure carry real programs, types, proofs, and rewrites.';
	const playgroundDesc = 'An in-browser Logos playground is on the way: evaluate expressions and watch the same engine the compiler uses rewrite them live.';
		...
		page({
			...
			jsonLd: [
				...
				},
				{
					...
					url: SITE_URL,
					downloadUrl: 'https://github.com/ThobiasKnudsen/LogosLang/releases',
					description: homeDesc,
	...
	await writePage(
		...
		page({
			...
			active: 'vision',
			path: '/vision/',
			description: 'Radical unification: why Logos puts programs, types, proofs, compilation, and the compiler itself in one structure.',
			main: visionPage(),
		})
	);
	await writePage(
		'examples/index.html',
		page({ title: 'Examples', active: 'examples', path: '/examples/', description: examplesDesc, main: placeholderPage('Examples', 'Worked examples are on the way, showing the one structure carrying real programs, types, proofs, and rewrites.') })
	);
	await writePage(
		'playground/index.html',
		page({ title: 'Playground', active: 'playground', path: '/playground/', description: playgroundDesc, main: placeholderPage('Playground', 'An in-browser playground is on the way. Evaluate expressions and watch the same engine the compiler uses rewrite them live.') })
	);
	...
	const marketing = [
		...
		{ path: '/vision/', title: 'Vision', desc: 'Radical unification: programs, types, proofs, compilation, and the compiler in one structure.' },
	...
	const docsLanding = docs.docEntries.length
		? [{ path: '/docs/', title: 'Documentation', desc: 'Logos documentation: getting started, guides, internals, and the operator reference.' }]
		: [];
	...
	console.log(`built ${docs.sectionCount} docs sections, versions [${docs.versions.join(', ')}] -> dist/`);
}
...
async function writeLlmsTxt(
	...
	const parts = [
		'# Logos (Λόγος)',
		'',
		'> Logos is a self-hosting systems programming language built on radical unification: programs, types, proofs, the optimizer, the standard library, and the compiler itself all live in one reflectable structure, the Logic Graph. The same operations that run a program can read, rewrite, optimize, and prove any of it, the compiler included.',
		'',
		'## Pages',
		'',
	...
	if (docEntries.length) {
		parts.push('', '## Documentation', '', [...docsLanding, ...docEntries].map(line).join('\n'));
	}
